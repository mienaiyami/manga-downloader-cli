...
export default class MangaKatana {
    async getChapters(url, start = 0, count = 0, spinner) {
        ...
        if (document.querySelector('meta[name="description"]')?.getAttribute("content")?.startsWith("Read")) {
            const mangaName = makeFileSafe(document.querySelector("#breadcrumb_wrap > ol > li:nth-child(2) > a > span")?.textContent ||
                "eeeeeeeee");
            const name = makeFileSafe(document.querySelector("#breadcrumb_wrap > ol > li:nth-child(3) > a > span")?.textContent ||
                "eeeeeeeee");
            const chapters = [{ name, url }];
            return { mangaName, chapters };
        }
        const mangaName = makeFileSafe(document.querySelector("#single_book > div.d-cell-medium.text > div > h1")?.textContent || "eeeeeeeeee");
        if (start < 0) {
            const tempData = [...document.querySelectorAll("div#single_book > .chapters a")].reverse()
                .map((e, i) => {
            ...
            [...document.querySelectorAll("div#single_book > .chapters a")]
                .reverse()
                .forEach((e, i) => {
                const chapterNumber = i;
